...
from nbconvert.exceptions import NBConvertMissingParameterException
from nbconvert.log import logger
from nbconvert.iorw import read_yaml_file
from nbconvert.translators import translate_parameters
from nbconvert.utils import find_first_tagged_cell_index
...
def parameterize_path(path, parameters):
    ...
    try:
        ...
        raise NBConvertMissingParameterException(f"Missing parameter {key_error}")
...
def parameterize_notebook(
    ...
    kernel_name = nbconvert_engines.nb_kernel_name(engine_name, nb, kernel_name)
    language = nbconvert_engines.nb_language(engine_name, nb, language)
    ...
    nb.metadata.nbconvert['parameters'] = parameters
