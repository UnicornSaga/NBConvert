...
from nbconvert import engines, exceptions
from nbconvert.engines import Engine, NBClientEngine, NotebookExecutionManager
...
class TestNotebookExecutionManager(unittest.TestCase):
    ...
    def test_get_cell_description(self):
        ...
        self.assertEqual(nb_man.get_cell_description(nb_man.nb.cells[1]), None)
    ...
    def test_notebook_start(self):
        ...
        self.assertEqual(nb_man.nb.metadata.nbconvert['start_time'], nb_man.start_time.isoformat())
        self.assertIsNone(nb_man.nb.metadata.nbconvert['end_time'])
        self.assertIsNone(nb_man.nb.metadata.nbconvert['duration'])
        self.assertIsNone(nb_man.nb.metadata.nbconvert['exception'])

        for cell in nb_man.nb.cells:
            self.assertIsNone(cell.metadata.nbconvert['start_time'])
            self.assertIsNone(cell.metadata.nbconvert['end_time'])
            self.assertIsNone(cell.metadata.nbconvert['duration'])
            self.assertIsNone(cell.metadata.nbconvert['exception'])
            self.assertEqual(cell.metadata.nbconvert['status'], NotebookExecutionManager.PENDING)
    ...
    def test_cell_start(self):
        ...
        self.assertEqual(cell.metadata.nbconvert['start_time'], fixed_now.isoformat())
        self.assertFalse(cell.metadata.nbconvert['exception'])
        self.assertEqual(cell.metadata.nbconvert['status'], NotebookExecutionManager.RUNNING)
    ...
    def test_cell_exception(self):
        ...
        self.assertEqual(nb_man.nb.metadata.nbconvert['exception'], True)
        self.assertEqual(cell.metadata.nbconvert['exception'], True)
        self.assertEqual(cell.metadata.nbconvert['status'], NotebookExecutionManager.FAILED)
    ...
    def test_cell_complete_after_cell_start(self):
        ...
        self.assertIsNotNone(cell.metadata.nbconvert['start_time'])
        start_time = dateutil.parser.parse(cell.metadata.nbconvert['start_time'])

        self.assertEqual(cell.metadata.nbconvert['end_time'], fixed_now.isoformat())
        self.assertEqual(cell.metadata.nbconvert['duration'], (fixed_now - start_time).total_seconds())
        self.assertFalse(cell.metadata.nbconvert['exception'])
        self.assertEqual(cell.metadata.nbconvert['status'], NotebookExecutionManager.COMPLETED)
    ...
    def test_cell_complete_after_cell_exception(self):
        ...
        self.assertEqual(cell.metadata.nbconvert['end_time'], fixed_now.isoformat())
        self.assertEqual(cell.metadata.nbconvert['duration'], (fixed_now - start_time).total_seconds())
        self.assertTrue(cell.metadata.nbconvert['exception'])
        self.assertEqual(cell.metadata.nbconvert['status'], NotebookExecutionManager.FAILED)
    ...
    def test_notebook_complete(self):
        ...
        self.assertIsNotNone(nb_man.nb.metadata.nbconvert['start_time'])
        start_time = dateutil.parser.parse(nb_man.nb.metadata.nbconvert['start_time'])

        self.assertEqual(nb_man.nb.metadata.nbconvert['end_time'], fixed_now.isoformat())
        self.assertEqual(nb_man.nb.metadata.nbconvert['duration'], (fixed_now - start_time).total_seconds())
        self.assertFalse(nb_man.nb.metadata.nbconvert['exception'])
    ...
    def test_notebook_complete_cell_status_with_failed(self):
        ...
        nb_man.notebook_complete()
        self.assertEqual(nb_man.nb.cells[0].metadata.nbconvert['status'], NotebookExecutionManager.COMPLETED)
        self.assertEqual(nb_man.nb.cells[1].metadata.nbconvert['status'], NotebookExecutionManager.FAILED)
...
class TestEngineBase(unittest.TestCase):
    ...
    def test_cell_callback_execute(self):
        ...
        with patch.object(NotebookExecutionManager, 'save') as save_mock:
            ...
            self.assertIsNotNone(nb.metadata.nbconvert['start_time'])
            self.assertIsNotNone(nb.metadata.nbconvert['end_time'])
            self.assertEqual(nb.metadata.nbconvert['duration'], AnyMock(float))
            self.assertFalse(nb.metadata.nbconvert['exception'])

            for cell in nb.cells:
                self.assertIsNotNone(cell.metadata.nbconvert['start_time'])
                self.assertIsNotNone(cell.metadata.nbconvert['end_time'])
                self.assertEqual(cell.metadata.nbconvert['duration'], AnyMock(float))
                self.assertFalse(cell.metadata.nbconvert['exception'])
                self.assertEqual(cell.metadata.nbconvert['status'], NotebookExecutionManager.COMPLETED)
...
class TestNBClientEngine(unittest.TestCase):
    ...
    def test_nb_convert_engine(self):
        with patch.object(engines, 'NBConvertNotebookClient') as client_mock:
            with patch.object(NotebookExecutionManager, 'save') as save_mock:
...
class TestEngineRegistration(unittest.TestCase):
    def setUp(self):
        self.nbconvert_engines = engines.nbconvert_engines

    def test_registration(self):
        mock_engine = Mock()
        self.nbconvert_engines.register("mock_engine", mock_engine)
        self.assertIn("mock_engine", self.nbconvert_engines._engines)
        self.assertIs(mock_engine, self.nbconvert_engines._engines["mock_engine"])

    def test_getting(self):
        mock_engine = Mock()
        self.nbconvert_engines.register("mock_engine", mock_engine)
        # test retrieving an engine works
        retrieved_engine = self.nbconvert_engines.get_engine("mock_engine")
        self.assertIs(mock_engine, retrieved_engine)
        # test you can't retrieve a non-registered engine
        self.assertRaises(exceptions.NBConvertlException, self.nbconvert_engines.get_engine, "non-existent")
    ...
    def test_registering_entry_points(self):
        ...
        with patch("entrypoints.get_group_all", return_value=[fake_entrypoint]) as mock_get_group_all:
            self.nbconvert_engines.register_entry_points()
            mock_get_group_all.assert_called_once_with("nbconvert.engine")
            self.assertEqual(self.nbconvert_engines.get_engine("fake-engine"), fake_entrypoint.load.return_value)
