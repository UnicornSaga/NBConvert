...
from nbconvert.iorw import write_ipynb
from nbconvert.log import logger
from nbconvert.utils import merge_kwargs, nb_kernel_name, nb_language, remove_args


class NBConvertEngines:
    """
    ...
    def register_entry_points(self):
        ...
        """
        for entrypoint in entrypoints.get_group_all("nbconvert.engine"):
    ...
    def get_engine(self, name=None):
        ...
        if not engine:
            raise NBConvertlException(f"No engine named '{name}' found")
        return engine
...
class NotebookExecutionManager:
    ...
    def notebook_start(self, **kwargs):
        ...
        self.nb.metadata.nbconvert['start_time'] = self.start_time.isoformat()
        self.nb.metadata.nbconvert['end_time'] = None
        self.nb.metadata.nbconvert['duration'] = None
        self.nb.metadata.nbconvert['exception'] = None
        ...
        for cell in self.nb.cells:
            ...
            # Clear out the nbconvert metadata for each cell.
    ...
    def cell_start(self, cell, cell_index=None, **kwargs):
        ...
        cell.metadata.nbconvert['start_time'] = self.now().isoformat()
        cell.metadata.nbconvert["status"] = self.RUNNING
        cell.metadata.nbconvert['exception'] = False
    ...
    def cell_exception(self, cell, cell_index=None, **kwargs):
        ...
        failure.
        """
        cell.metadata.nbconvert['exception'] = True
        cell.metadata.nbconvert['status'] = self.FAILED
        self.nb.metadata.nbconvert['exception'] = True
    ...
    def cell_complete(self, cell, cell_index=None, **kwargs):
        ...
        cell.metadata.nbconvert['end_time'] = end_time.isoformat()
        if cell.metadata.nbconvert.get('start_time'):
            start_time = dateutil.parser.parse(cell.metadata.nbconvert['start_time'])
            cell.metadata.nbconvert['duration'] = (end_time - start_time).total_seconds()
        if cell.metadata.nbconvert['status'] != self.FAILED:
            cell.metadata.nbconvert['status'] = self.COMPLETED
    ...
    def notebook_complete(self, **kwargs):
        ...
        self.nb.metadata.nbconvert['end_time'] = self.end_time.isoformat()
        if self.nb.metadata.nbconvert.get('start_time'):
            self.nb.metadata.nbconvert['duration'] = (self.end_time - self.start_time).total_seconds()

        # Cleanup cell statuses in case callbacks were never called
        for cell in self.nb.cells:
            if cell.metadata.nbconvert['status'] == self.FAILED:
                break
            elif cell.metadata.nbconvert['status'] == self.PENDING:
                cell.metadata.nbconvert['status'] = self.COMPLETED
    ...
    def get_cell_description(self, cell, escape_str="nbconvert_description="):
...
class NBClientEngine(Engine):
    ...
    def execute_managed_notebook(
        ...
        **kwargs,
        ...
        return NBConvertNotebookClient(nb_man, **final_kwargs).execute()


# Instantiate a NBConvertEngines instance, register Handlers and entrypoints
nbconvert_engines = NBConvertEngines()
nbconvert_engines.register(None, NBClientEngine)
nbconvert_engines.register('nbclient', NBClientEngine)
nbconvert_engines.register_entry_points()
