...
from nbconvert import iorw
from nbconvert.exceptions import NBConvertlException
from nbconvert.iorw import (
    HttpHandler,
    LocalHandler,
    NoIOHandler,
    NotebookNodeHandler,
...
class TestNBConvertIO(unittest.TestCase):
    """
    ...
    def setUp(self):
        self.nbconvert_io = NBConvertIO()
        self.nbconvert_io_bytes = NBConvertIO()
        self.fake1 = self.FakeHandler(1)
        self.fake2 = self.FakeHandler(2)
        self.fake_byte1 = self.FakeByteHandler(1)
        self.nbconvert_io.register("fake", self.fake1)
        self.nbconvert_io_bytes.register("notebooks", self.fake_byte1)

        self.old_nbconvert_io = iorw.nbconvert_io
        iorw.nbconvert_io = self.nbconvert_io

    def tearDown(self):
        iorw.nbconvert_io = self.old_nbconvert_io

    def test_get_handler(self):
        self.assertEqual(self.nbconvert_io.get_handler("fake"), self.fake1)

    def test_get_local_handler(self):
        with self.assertRaises(NBConvertlException):
            self.nbconvert_io.get_handler("dne")

        self.nbconvert_io.register("local", self.fake2)
        self.assertEqual(self.nbconvert_io.get_handler("dne"), self.fake2)

    def test_get_no_io_handler(self):
        self.assertIsInstance(self.nbconvert_io.get_handler(None), NoIOHandler)

    def test_get_notebook_node_handler(self):
        test_nb = nbformat.read(get_notebook_path('test_notebooknode_io.ipynb'), as_version=4)
        self.assertIsInstance(self.nbconvert_io.get_handler(test_nb), NotebookNodeHandler)
    ...
    def test_entrypoint_register(self):
        ...
        with patch("entrypoints.get_group_all", return_value=[fake_entrypoint]) as mock_get_group_all:
            self.nbconvert_io.register_entry_points()
            mock_get_group_all.assert_called_once_with("nbconvert.io")
            fake_ = self.nbconvert_io.get_handler("fake-from-entry-point://")
            assert fake_ == fake_entrypoint.load.return_value

    def test_register_ordering(self):
        # Should match fake1 with fake2 path
        self.assertEqual(self.nbconvert_io.get_handler("fake2/path"), self.fake1)

        self.nbconvert_io.reset()
        self.nbconvert_io.register("fake", self.fake1)
        self.nbconvert_io.register("fake2", self.fake2)

        # Should match fake1 with fake1 path, and NOT fake2 path/match
        self.assertEqual(self.nbconvert_io.get_handler("fake/path"), self.fake1)
        # Should match fake2 with fake2 path
        self.assertEqual(self.nbconvert_io.get_handler("fake2/path"), self.fake2)

    def test_read(self):
        self.assertEqual(self.nbconvert_io.read("fake/path"), "contents from fake/path for version 1")

    def test_read_bytes(self):
        self.assertIsNotNone(self.nbconvert_io_bytes.read("notebooks/gcs/gcs_in/gcs-simple_notebook.ipynb"))

    def test_read_with_no_file_extension(self):
        with pytest.warns(UserWarning):
            self.nbconvert_io.read("fake/path")

    def test_read_with_invalid_file_extension(self):
        with pytest.warns(UserWarning):
            self.nbconvert_io.read("fake/path/fakeinputpath.ipynb1")

    def test_read_with_valid_file_extension(self):
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            self.nbconvert_io.read("fake/path/fakeinputpath.ipynb")
    ...
    def test_listdir(self):
        self.assertEqual(self.nbconvert_io.listdir("fake/path"), ["fake", "contents"])

    def test_write(self):
        self.assertEqual(self.nbconvert_io.write("buffer", "fake/path"), "wrote buffer")

    def test_write_with_no_file_extension(self):
        with pytest.warns(UserWarning):
            self.nbconvert_io.write("buffer", "fake/path")

    def test_write_with_path_of_none(self):
        self.assertIsNone(self.nbconvert_io.write('buffer', None))

    def test_write_with_invalid_file_extension(self):
        with pytest.warns(UserWarning):
            self.nbconvert_io.write("buffer", "fake/path/fakeoutputpath.ipynb1")

    def test_pretty_path(self):
        self.assertEqual(self.nbconvert_io.pretty_path("fake/path"), "fake/path/pretty/1")
...
class TestLocalHandler(unittest.TestCase):
    ...
    def test_local_file_io_cwd(self):
        with TemporaryDirectory() as temp_dir:
            ...
            handlers = nbconvert_io._handlers
            try:
                local_handler = LocalHandler()
                nbconvert_io.reset()
                nbconvert_io.register("local", local_handler)
                ...
                nbconvert_io.handlers = handlers
...
class TestHttpHandler(unittest.TestCase):
    ...
    def test_listdir(self):
        ...
        """
        with self.assertRaises(NBConvertlException) as e:
    ...
    def test_read(self):
        ...
        with patch('nbconvert.iorw.requests.get') as mock_get:
    ...
    def test_write(self):
        ...
        with patch('nbconvert.iorw.requests.put') as mock_put:
