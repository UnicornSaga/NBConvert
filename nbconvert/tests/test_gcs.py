...
from nbconvert.exceptions import NBConvertRateLimitException
from nbconvert.iorw import GCSHandler, fallback_gs_is_retriable
...
class GCSTest(unittest.TestCase):
    ...
    @patch('nbconvert.iorw.GCSFileSystem', side_effect=mock_gcs_fs_wrapper())
        ...
        with patch.object(GCSHandler, 'RETRY_DELAY', 0):
            with patch.object(GCSHandler, 'RETRY_MULTIPLIER', 0):
                with patch.object(GCSHandler, 'RETRY_MAX_DELAY', 0):
                    self.assertRaises(GCSRateLimitException, self.gcs_handler.write, 'raise_limit_exception', 'gs://bucket/test.ipynb')
    ...
    @patch('nbconvert.iorw.gs_is_retriable', side_effect=fallback_gs_is_retriable)
    @patch(
        'nbconvert.iorw.GCSFileSystem',
    ...
    @patch('nbconvert.iorw.gs_is_retriable', return_value=False)
    @patch(
        'nbconvert.iorw.GCSFileSystem',
        side_effect=mock_gcs_fs_wrapper(GCSRateLimitException({"message": "test", "code": 500}), 1),
    )
